...
def category_categories(request, slug, start=0, template_name="lfs/catalog/categories/category/default.html"):
    ...
    for i, children in enumerate(category.get_children()):
        products = lfs.catalog.models.Product.objects.filter(categories__in=[children]).distinct()
        for product in products:
            product_image = product.get_image()
            if product_image:
                product.image = product_image.image
        children.subcategories_products = products
        row.append(children)
...
def category_products(request, slug, start=1, template_name="lfs/catalog/categories/product/default.html"):
    ...
    if category.get_children():
        ...
        category.filter_categories.extend(category.get_children())
    else:
    ...
    if manufacturerId:
        ...
        all_products = all_products.filter(manufacturer=manufacturer)
    # prepare paginator
